...
`MVC`는 관심사가 엄격하게 분리된 `Layered` 이며 내부 동작은 그 계층에 격리되기 때문에 인접한 인터페이스를 통해서만 소통할 수 있습니다. 캡슐화라고 합니다. 쉽게 말해 `View` 에서 `Model` 에 직접 접근할 수 없으며, 반드시 `Controller` 를 통해서만 작업을 완료할 수 있다는 뜻입니다. 이것은 뷰와 모델 사이를 완충해줄 수 있는 추상계층을 하나 만드는 것 같습니다.
...
우리가 객체를 만들면 거의 대부분 private 로 선언합니다. 자신이 하는 세부 작업은 숨기죠. 자신이 품고 있는 도우미 객체를 숨기고, 자신이 취급하는 자료타입도 숨깁니다. 상위 레이어에서 요구하는 인터페이스만 구현하여 공개합니다. 또 상위 레이어와 합의된 데이터 타입으로 반환하게 됩니다. 우리는 경험을 통해 알고 있습니다. 뭐든 다 교체될 수 있다는 것을요.
