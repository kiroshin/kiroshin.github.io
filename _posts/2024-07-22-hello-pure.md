...
쇼가 유틸을 사용할 수도 있고, 간혹 기어가 컴포넌트를 만들어주는 경우도 있을 것입니다(이럴 경우 기어는 싱글톤으로 쓰이거나 단순 함수 형태일 것입니다). 가시권이 시스템 내부를 향하지 않는 독립 컴포넌트로 목적별로 형태가 아주 다양할 것입니다. 이같은 수많은 컴포넌트를 Ui Controller 에서 끌어와 조립하고 바인딩 값을 넣어주는 건 약간 번거로울 수 있습니다. 중간 컴포넌트를 만들어 주면 어느 정도 수월하긴 합니다. Trade-off 관계이긴 하지만 Ui Controller 가 이전보다 훨씬 가벼워지기 때문에 유지보수에는 효과가 더 좋습니다.
