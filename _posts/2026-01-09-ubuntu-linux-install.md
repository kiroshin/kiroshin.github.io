...
클라우드가 생긴 이후로 직접 서버를 인스톨할 일이 거의 없어졌습니다. 구형 PC가 남아 돌아도 물리머신을 돌리면 전기도 먹고 팬 소음도 있어서 이걸 24시간 구동하는 일이 없어요. 그런데 가끔 필요할 때가 있습니다. 가끔요. 테스트 머신이 필요할 때인데, 본격 서버에서 해보기에는 부담이 되거나 클라우스에서 무료로 해보기에는 시스템 자원이 너무 척박한 경우 필요해집니다. 그래서 며칠 실험용으로 쓸 구형PC 한 대를 꺼냈습니다. 10년 된 것인데 클라우드에서 무료로 얻을 수 있는 사양과 비교할 수 없이 좋습니다. 데비안 계열의 우분투만 사용해봐서 고민없이 우분투 서버 LTS를 설치하기로 했습니다.
...
네트워크는 수동으로 잡았습니다. 공유기에 물려 있다 하더라도 서버는 DHCP가 별 의미 없습니다. 나중에 모니터를 빼고 터미널을 통해 접근하려면 수동IP 가 속편합니다. 위 설정에서 다만 조심해야 할 것은 서브넷 마스크인데요. 윈도우에서 하듯이 255.255.255.0 으로 마스킹을 하면 안 됩니다. CIDR 표기법으로 넣어야 합니다. 그외 네임서버는 따로 없으니까 구글 꺼로 넣었고, 이 테스트 머신에 도메인 따위를 할당하지 않았기 때문에 Search domains 는 비워둬도 됩니다.
...
현재 컴퓨터에는 웹스턴 하드와 삼성 하드가 물려 있습니다. 하드디스크 파티션은 잘 봐야 합니다. 하드가 털털 남는데도 WD 하드가 ubuntu-vg에 100G 만 할당되었습니다. 아무 생각 없이 엔터치면 설치 다 됐을 때 당황하게 됩니다. 저는 전체를 다 사용할 것이므로 아래처럼 디스크의 남는 용량을 직접 넣어줘야 합니다. max 에 뜨는 숫자를 키보드로 그대로 넣어주면 됩니다.
